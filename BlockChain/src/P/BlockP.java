...
public class BlockP {//start the process(s) and stuff
			...
			try {
	            ...
	            String prevHash= "";
	              ...
	              try {
		                ...
		                //extra try inside helps deal with exceptions since we're reading stuff in
		                try {
		                	while((line_read = buffread.readLine()) != null) {//use loop to go through each line of the file and split them into arrays
			                	...
			                	BlockMaker newBlock = new BlockMaker(pNum, "Block "+pNum+"-"+blocks_n, prevHashId, data, blocks_n,data[0], data[1], data[2], data[5], data[3], data[4], data[6], 1,UUID.randomUUID(), dF.format(new Date()));//fill in a block with what you stored in data array
			                	
			                	String encrypted_Hash_ID = BlockMethod_utilities.hexString(u.encrypt(newBlock.calcHashID(), u.privateKey()));//gets instance of json handling class and changes this to hex because bytes and stuff are difficult to read. We use private key to encrypt the hash
			                	newBlock.set_HashID(encrypted_Hash_ID);//use private key as sig and set block 's hashID to an encrypted one
			                	
			                	System.out.println("Add timestamped blocks... "+newBlock.getTimeStamp()+": "+newBlock.getFirstName()+" "+newBlock.getLastName());//DEBUG for prompting console
...
class Unverified_BlockReceiver_class extends Thread {//handles the unverified blocks
	 //since we're receiving stuff we need instances of BlockHandler and BLockMethod_utilities
	//BlockHandler to handle encryption/decryption related stuff
	//BlockMethod_utilities to handle multicasting related stuff 
	private BlockHandler pH;
	  private BlockMethod_utilities uT;
	  private int sourceProcessNum;
...
class Unverified_BlockReceiver_Consumer extends Thread{
	...
	public void run() {
		if(pH.waitForKeys()) {//wait until all public keys get sent out to necessary processes before doing anything
			...
			while(pH.waitForBlocks()) {
				...
				if(uT.decrypt(BlockMethod_utilities.byteArrayfromHex(earliest_block.getHashID()), pH.publicKey(earliest_block.getOwnerProcess())).equals(earliest_block.calcHashID())) {
					//we have to verify sig from process matches calculated block hash
					System.out.println("Verified! "+earliest_block.getBlockID()+": "+earliest_block.getFirstName()+" "+earliest_block.getLastName());
				}
				else {
					System.out.println("INVALID block: "+earliest_block.getBlockID()+": "+earliest_block.getFirstName()+" "+earliest_block.getLastName());
				}//else
			}//while
		}//if
	}//run
...
class BlockMaker{//make the blocks here
	...
	private String prevHashID;
	  ...
	  private int ownerP;//helps us for verification of sig
	  
	  private String[] transactions;
	  
	  private boolean verified;
	  
	  public BlockMaker(int ownerP, String BlockID, String prevHashID, String[] transactions, int blockNumber, String firstName, String lastName, String dob, String Rx, String ssNumber, String illness, String treatment, int proofOfWORK, UUID uuid, String timeStamp) {
	...
	//VERIFY------------------
	  ...
	  public void set_HashID(String updatedHash) {
		  hashID = updatedHash;
	  }
	...
	//SHA-256 digest of the block to get hash--------
	  public String calcHashID(){//generate a hash id using all the block's given properties
	      ...
	      sb.append(this.ownerP);//to identify which block it is
	      sb.append(this.prevHashID);
	      sb.append(this.nonce_randomSeed);
	      sb.append(Arrays.toString(this.transactions));
	      ...
	      return BlockMethod_utilities.hexString(byteData);
...
class BlockHandler{
	...
	BlockHandler(int processNum) {//constructor
		processNumber=processNum;
	      ...
	      isRunning=true;//process is up
	      uT=new BlockMethod_utilities(1024,processNumber);
	...
	//-----------waiting methods-----------
	  public synchronized boolean wait_For_Last_Process_Typed_In_From_arg() {//wait and check notifications; returns true when received key from last process (usually Process 2 since we usually type in 2 last)
		...
		System.out.println("Process "+(BlockMethod_utilities.maxProcesses-1)+" is now up and running");//DEBUG
		return(isRunning);                                      
	  }
	...
	//PQ related methods
	  ...
	  public synchronized boolean waitForBlocks() {//true if we receive all 12 blocks
		      ...
		      while(isRunning && block.isEmpty()) {
		          try {
		              wait();//we need notification before we're up and running again
		          } catch(InterruptedException e) {
...
class BlockMethod_utilities {//go from java to json in  this class
	...
	BlockMethod_utilities(int keySize, int pNum) {//constructor with basic parameters for key size and process number
	      ...
	      for(int socketType=publicKeySocket;socketType<=updatedBlockChainSocket;socketType++) {
	          for(int i=0;i<maxProcesses;i++) {//for loop deals with process number here
	...
	//cipher utilities
	  ...
	  public BlockMaker blockFromJson(String s) {//go from json to java
	      try {
	          return(gson.fromJson(s.trim(),BlockMaker.class));
	      } 
	      catch(JsonSyntaxException e) {
	          e.printStackTrace();//something went wrong when trying to go from json to java
	      }
	      return(null);
	  }
	//---------------JSON related stuff---------------
	  ...
	  //for hex converting in between to make it a bit more readable to humans
	  public static String hexString(byte[] byte_data) {//make things more readable by converting to hexcode since bytes aren't that readable to humans
		  StringBuffer bf = new StringBuffer();
		  for(int i = 0; i < byte_data.length; i++) {
			  bf.append(Integer.toString((byte_data[i] & 0xff) + 0x100, 16).substring(1));
		  }
		  return bf.toString();
	  }
	  
	  public static byte[] byteArrayfromHex(String hexStringToBytes) {//we go from hexcode back to bytes to make it easier to manipulate elsewhere
		  if(hexStringToBytes.length()%2==1) {
			  //if odd
			  System.out.println("We have an odd length, so what's returned from this function is null.");
			  return null;
		  }
		  else {
			  byte[] bData=new byte[hexStringToBytes.length()/2];
	            for(int i=0;i<hexStringToBytes.length()/2;i++) {
	                bData[i]=(byte) Integer.parseInt(hexStringToBytes.substring(2*i, 2*i+2),16);
	            }
	            return bData;
		  }
	  }
}//BlockMethod_utilites